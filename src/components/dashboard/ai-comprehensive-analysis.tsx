...
const MiniChart = ({ data, suppliers, t }: { data: ChartDataPoint[], suppliers: Supplier[], t: (key: string) => string }) => (
...
export default function AiComprehensiveAnalysis({ suppliers, chartData }: AiComprehensiveAnalysisProps) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>{t('aiAnalysis.comprehensive.cardTitle')}</CardTitle>
        <CardDescription>
          {t('aiAnalysis.comprehensive.cardDescription')}
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4">
          <Button onClick={handleAnalyze} disabled={isPending || suppliers.length === 0}>
            ...
            {t('aiAnalysis.comprehensive.button')}
          </Button>
        ...
        {isPending && (
          <div className="flex items-center justify-center gap-3 text-lg text-muted-foreground py-10">
            ...
            <p>{t('aiAnalysis.comprehensive.pending')}</p>
          </div>
        )}
        ...
        {hasAnalysis && (
          <div className="space-y-4">
            <Alert>
              ...
              <AlertTitle>{t('aiAnalysis.comprehensive.doneTitle')}</AlertTitle>
              <AlertDescription>
                {t('aiAnalysis.comprehensive.doneDescription')}
              </AlertDescription>
            </Alert>
                ...
                {analysisItems.map((item, index) => (
                    <Card key={index} className="flex flex-col">
                        ...
                        <CardContent className="flex-1 flex flex-col">
                            <MiniChart data={item.data} suppliers={suppliers} t={t} />
                            <div 
